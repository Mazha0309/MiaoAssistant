// Adapted from KernelSU's Apache-2.0 liquid-glass helpers, which are based on
// Kyant0/AndroidLiquidGlass. The shader is kept local so the visual effect is real
// refraction rather than a translucent color pretending to be glass.
...
internal fun BackdropEffectScope.lens(
    ...
    val key = if (dispersionEnabled) "MiaoLiquidGlassLensDispersion" else "MiaoLiquidGlassLens"
...
half4 main(float2 coord) {
    ...
    float2 dispersedCoord = d * grad * dispersion;
